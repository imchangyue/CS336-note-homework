...
def run_memory_profiling(args):
    ...
    # 总是使用 bfloat16 精度进行分析，如果设备支持
    if torch.cuda.is_bf16_supported():
        print("自动启用 bfloat16 精度进行内存分析。")
        model.to(torch.bfloat16)
    else:
        print("警告: 当前 GPU 不支持 bfloat16，将使用默认的 float32 精度。")
    ...
    output_filename = f"memory_snapshot/memory_snapshot_{args.model_size}_{args.mode}_{args.block_size}_bf16.pickle"
